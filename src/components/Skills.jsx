  ...
  return (
    ...
    >
      <div className="flex justify-between items-center mb-2">
        <span className="text-white font-medium text-sm font-sans">{skill.name}</span>
        <span className="text-[#ff6a00] font-heading font-bold text-sm">{skill.level}%</span>
      </div>
      <div className="h-2 bg-[#1a1a2e] border border-white/5 overflow-hidden">
        ...
        >
          ...
          <motion.div
            animate={{ x: ['-100%', '200%'] }}
            transition={{ duration: 2, delay: index * 0.06 + 1.5, ease: 'easeInOut' }}
...
export default function Skills() {
  ...
  return (
    <section id="skills" className="relative py-24 bg-[#0d0d14] overflow-hidden">
      {/* Background text */}
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 lg:px-16 relative z-10">
        ...
        >
          ...
          <div className="text-[#ff6a00] font-bold text-sm uppercase tracking-widest mb-3">Expertise</div>
          ...
          <p className="text-gray-400 text-lg mt-4 max-w-2xl mx-auto font-sans">
            Proficiency across civil engineering tools, technical execution, and specialized domains.
          </p>
        </motion.div>
            ...
            >
              ...
              <div className="flex flex-col gap-5">
                {groupedSkills[category].map((skill, i) => (
                  <SkillBar key={skill.name} skill={skill} index={i + catIndex * 4} />
                ))}
              </div>
            </motion.div>
          ))}
        ...
        >
          {skills.map((skill) => (
            <motion.span
              key={skill.name}
              whileHover={{ scale: 1.1, backgroundColor: '#ff6a00', color: '#fff' }}
