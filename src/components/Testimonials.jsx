...
function TestimonialCard({ testimonial, highlight = false }) {
  return (
    <>
      {/* Quote icon */}
      <MdFormatQuote
        className={`text-5xl mb-4 ${highlight ? "text-[#ff6a00]" : "text-white/20"}`}
      />
      ...
      <div className="flex items-center gap-3 border-t border-white/10 pt-4">
        <div
          className={`w-10 h-10 flex items-center justify-center font-heading font-black text-sm flex-shrink-0 ${
            highlight
              ? "bg-[#ff6a00] text-white"
              : "bg-[#1a1a2e] text-[#ff6a00]"
          }`}
        >
